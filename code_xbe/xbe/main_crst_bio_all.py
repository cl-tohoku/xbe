    ...
    for i in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            if args.dataset == "semeval" or args.dataset == "tacred":
                ...
                eval_func = eval_ACC
            elif args.dataset == "bio" or args.dataset == "gids":
                eval_func = eval_AP
            ...
            if score > best_dev_score:
                ...
                if args.freeze_kg:
                    temp = '_bag_kg_frozen_crst_bio_%s.mdl'%args.crst_mod if args.use_bag else '_kg_frozen_crst_bio_%s.mdl'%args.crst_mod
                else:
                    temp = '_bag_kg_crst_bio_%s.mdl'%args.crst_mod if args.use_bag else '_kg_crst_bio_%s.mdl'%args.crst_mod
                if args.direct_feature:
                    temp = '_direct_'+temp
                if args.use_seg:
                    temp = '_seg'+temp 
                if args.ckpt_to_load != 'None':
                    torch.save(model.state_dict(), '../../save/bio/'+args.prefix+args.cross_layers+str(args.w_symloss)+args.dataset+'_'+args.ckpt_to_load+'_'+args.kg_method+temp)
                else:
                    torch.save(model.state_dict(), '../../save/bio/'+args.prefix+args.cross_layers+str(args.w_symloss)+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp)
            else:
                print("Dev score: %.3f" % score)
            print("-----------------------------------------------------------") 
    print("@RESULT: " + args.dataset +" Test score is %.3f" % best_test_score)
    f = open("../log/re_log_bio", 'a+')
    temp = '_bag_kg_crst_bio_%s.mdl'%args.crst_mod if args.use_bag else '_kg_crst_bio_%s.mdl'%args.crst_mod
    if args.direct_feature:
        temp = '_direct_'+temp
    if args.use_seg:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--surf_trf", type=str,
			default="dmis-lab/biobert-v1.1", help="'bert-base-uncased' or 'dmis-lab/biobert-v1.1'")
    ...
    parser.add_argument("--cross_layers", type=str,
                        default='', help="in the format of 4_5")
    parser.add_argument("--dev_bio", action='store_true',
                        default=False, help="testing on bio dev set")
    ...
    if args.test or args.load:
        ...
        if args.ckpt_to_load != 'None':
            ...
            loaded = torch.load("../../save/bio/"+args.prefix+args.cross_layers+str(args.w_symloss)+args.dataset+'_'+args.ckpt_to_load+'_'+args.kg_method+temp)
        else:
            print('loading',args.prefix+str(args.w_symloss)+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp)
            loaded = torch.load("../../save/bio/"+args.prefix+args.cross_layers+str(args.w_symloss)+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp)
    ...
    if args.test:
        ...
        if args.ckpt_to_load != 'None':
            if args.freeze_kg:
                temp = '_bag_kg_frozen_crst_bio_%s_'%args.crst_mod if args.use_bag else '_kg_frozen_crst_bio_%s_'%args.crst_mod
            else:
                temp = '_bag_kg_crst_bio_%s_'%args.crst_mod if args.use_bag else '_kg_crst_bio_%s_'%args.crst_mod
            if args.direct_feature:
                temp = '_direct_'+temp
            if args.use_seg:
                temp = '_seg'+temp 
            np.save('../../result/bio/'+args.prefix+args.dataset+'_'+args.ckpt_to_load+'_'+args.kg_method+temp+'logits.npy', tot_logits)
            np.save('../../result/bio/'+args.prefix+args.dataset+'_'+args.ckpt_to_load+'_'+args.kg_method+temp+'labels.npy', tot_labels)
        else:
            if args.freeze_kg:
                temp = '_bag_kg_frozen_crst_bio_%s_'%args.crst_mod if args.use_bag else '_kg_frozen_crst_bio_%s_'%args.crst_mod
            else:
            ...
            if args.use_seg:
                temp = '_seg'+temp 
            np.save('../../result/bio/'+args.prefix+args.cross_layers+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp+'logits.npy', tot_logits)
            np.save('../../result/bio/'+args.prefix+args.cross_layers+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp+'labels.npy', tot_labels)

            # tx info.
            if args.output_example:
                with open('../../result/bio/'+args.prefix+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp+'sample.txt', 'w') as fout:
                    for sample in tot_sample_tup_pred:
                        fout.write('%s\n' % sample)
            if args.output_att and args.crst_mod == 'resi':
                with open('../../result/bio/'+args.prefix+args.dataset+'_'+'bert-base-uncased'+'_'+args.kg_method+temp+'att_info.json', 'w') as fout:
